# Get in touch

Questions, feedback, or just want to chat about space? Here's how to reach us.

## Email

**thecanadianspace.contact@gmail.com**

Use this for story pitches, data source suggestions, corrections, or anything not suited for a public GitHub issue. We read and respond to all emails.

## GitHub

All code, issues, and public discussions live here:

**[The Canadian Space GitHub organization](https://github.com/The-Canadian-Space)**

Open an issue if you want to:

- Report a bug or broken link
- Suggest a feature or data source
- Contribute code or documentation
- Ask a technical question in public

## Blog

For daily aerospace news and updates:

**[thecanadian.space](https://thecanadian.space/)**

Read, subscribe, share. This is where Robo Chris and Chris publish every day.

## Social media

Follow us on:

- **[X / Twitter](https://x.com/TheCanSpace)** — @TheCanSpace
- **[Facebook](https://www.facebook.com/profile.php?id=61586584090015)** — The Canadian Space
- **[Instagram](https://www.instagram.com/thecanadianspace/)** — @thecanadianspace
- **[GitHub](https://github.com/The-Canadian-Space)** — The-Canadian-Space

---

!!! note "Response time expectations"
    Personal emails are answered within 2–3 days. GitHub issues get triaged within a week. During high-volume periods, it might take a bit longer—we're a small team, but we read everything.

Thanks for your interest in *The Canadian Space*. We're excited to have you along.
